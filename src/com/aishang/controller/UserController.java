...
//		request.setCharacterEncoding("UTF-8");
	...
	private void update(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(c==1){
			...
			response.sendRedirect("/java1910/userController?flag=userList&pageNow="+pageNow+"&searchName="+searchName+"&searchDep="+searchDep+"");
			return;
		}else{
			//修改失败
	...
	//获取登陆人的id
		...
		if(i>0){
			response.sendRedirect("/java1910/userController?flag=userList&pageNow="+pageNow+"&searchName="+URLEncoder.encode(searchName, "UTF-8")+"&searchDep="+searchDep+"");
		}else{
			String msg1 = "不能删除自己或已有用户已发表新闻";
			msg1 = URLEncoder.encode(msg1, "UTF-8");
			response.sendRedirect("/java1910/userController?flag=userList&pageNow="+pageNow+"&searchName="+URLEncoder.encode(searchName, "UTF-8")+"&searchDep="+searchDep+"&msg1="+msg1+"");
		}
	}
	...
	private void jump(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		response.sendRedirect("/java1910/userController?flag=userList&pageNow="+numb+"&searchName="+searchName+"&searchDep="+searchDep+"");
	}
	...
	//创建User
		...
		if(username==null || "".equals(username)){
			request.setAttribute("errName", "大神，请输入用户名");
			request.setAttribute("password", password);
			request.setAttribute("age", age);
			request.getRequestDispatcher("updateUser.jsp").forward(request, response);
		}else if(password==null || "".equals(password)){
			request.setAttribute("errPass", "大神，请输入密码");
			...
			request.setAttribute("username", username);
			request.setAttribute("findDepartment", findDepartment);
			request.setAttribute("errAge", "大神，请输入年龄");
			request.getRequestDispatcher("updateUser.jsp").forward(request, response);
		}else if(age<=0){
			request.setAttribute("password", password);
			request.setAttribute("username", username);
			request.setAttribute("findDepartment", findDepartment);
			request.setAttribute("errAge", "大神，别闹！");
			request.getRequestDispatcher("updateUser.jsp").forward(request, response);
		}else{
			//执行修改
			UserService userService = new UserService();
			...
			if(i>0){
				//修改成功
				response.sendRedirect("/java1910/skip");
			}else{
				//修改失败
	...
	private void doLogin(HttpServletRequest request,
		...
		if(checkUser!=null || "".equals(checkUser)){
			...
			//跳转到主页
			response.sendRedirect("/java1910/userController?flag=userList");
			return;
		}else{
			//登录失败
