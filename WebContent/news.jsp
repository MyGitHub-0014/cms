						...
						<div class="col-md-12">
							...
							<form action="newsController" method="post">
		                           ...
		                           <div style="float:left; margin:0 2px; width: 100px;">
			                           ...
			                           <option  value="0" ${news.flag eq 0?"selected='selected'":"" }>未审核</option>
			                           <option  value="1" ${news.flag eq 1?"selected='selected'":"" }>未通过</option>
			                           <option  value="2" ${news.flag eq 2?"selected='selected'":"" }>已通过</option>
			                           </select>
		                           </div>
													...
													<tr>
														...
														<td>
															<select class="form-control" name="upPend" id="upPend" onchange="upd(${newsList.id })">
																	<option value="0" ${newsList.flag eq 0?"selected":"" } >未审核</option>
																	<option value="1" ${newsList.flag eq 1?"selected":"" } >未通过</option>
																	<option value="2" ${newsList.flag eq 2?"selected":"" } >已通过</option>
															</select>
														</td>
														
															<td>
															<button type="button" rel="tooltip" title="修改" class="btn btn-info btn-simple btn-xs" onclick="window.location.href='${pageContext.request.contextPath}/newsController?flag=updateEcho&id=${newsList.id }&seltypeID=${news.typeid}&selPend=${news.flag}&selNewsName=${news.title}&fla=${newsList.flag }&pageNow=${pageNow}'"> 
