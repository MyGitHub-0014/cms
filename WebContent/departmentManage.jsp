...
<script type="text/javascript">
	$(function(){
		$.ajax({
			url:"${pageContext.request.contextPath}/departmentController?flag=findDepartment",
			type:"get",
			dataType:"json",
	...
	function upd(so){
		$.ajax({
			url:"${pageContext.request.contextPath}/departmentController?flag=updateDepartment",
			type:"get",
			data:{
	...
	function echo(ec){
		$.ajax({
			...
			success:function(data){
				if(data == "no"){
					$($(ec).next()).html("<font color='red'>部门名称不可为空</font>");
					$($(ec).parent().next().next().find("button")).attr("disabled",true);
				}else if(data == "typeno"){
					$($(ec).next()).html("<font color='red'>部门重复</font>");
					$($(ec).parent().next().next().find("button")).attr("disabled",true);
				}else{
	...
	function sor(so){
		$.ajax({
			...
			success:function(data){
				...
				if(data == "no"){
					$($(so).parent().next()).html("<font color='red'>部门优先级不可为空</font>");
					$($(so).parent().next().parent().next().find("button")).attr("disabled",true);
				}else if(data == "errSortno"){
					$($(so).parent().next()).html("<font color='red'>部门优先级重复</font>");
					$($(so).parent().next().parent().next().find("button")).attr("disabled",true);
				}else{
...
function delDepartment(del){
	$.ajax({
		url:"${pageContext.request.contextPath}/departmentController?flag=delDepartment",
		type:"get",
		data:{
			typeID:del,
			con:confirm("确认删除?"),
...
<!-- 部门名称ajax重复判断 -->
	<script type="text/javascript">
		$(function(){
			$("#typeName").blur(function(){
				$.ajax({
					url:"${pageContext.request.contextPath}/departmentController?flag=ajaxTypeName",
					type:"get",
					data:{
					...
					success:function(data){
						console.log(data)
						if(data=="ok"){
							$("#errTypeName").html("<font color='red'>部门名称重复</font>");
	...
	function check(){
		var c = true;
		if($("#typeName").val()==""){
			$("#errTypeName").html("<font color='red'>请输入部门名称</font>");
			c=false;
		}
		if($("#sort").val()==""){
                            ...
                            <div class="content">
                                ...
                                <div class="footer">
                                    ...
                                    <div class="stats">
                                        ...
                                        <form action="${pageContext.request.contextPath }/departmentController?flag=addDepartment" method="post" onsubmit="return check()">
                                        <table class="table">
                                            <tr>
                                                <td>      
                                                ...
                                                <td>      
					                            <input type="text" class="form-control" name="typeName" id="typeName" placeholder="部门名称"  value="${typeName }">
					                            <span id="errTypeName" style='color:red'>${errTypeName }</span>
					                            </td>
					                            <td>
					                            <div style=" width:130px;">
												<input type="text" class="form-control" placeholder="部门优先级" name="sort" id="sort" value="${sort }" >
                                                ...
                                                <td class="td-actions text-right">
		                          				...
		                          				<button type="submit" id="sb" class="btn btn-info btn-fill pull-right" >添加部门</button>
